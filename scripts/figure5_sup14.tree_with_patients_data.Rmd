...
get_tree_data <- function(species, diff_threshold=0.0001){
    ...
    dist.dat <- read.table(paste0('../tables/',species,'_opera_ms_mummer_dist.dat')) %>% 
      select(V1, V2, coverage=V7, ngsnp=V9, identity=V10)
    ...
    tmp <- 
      ...
      filter(coverage>0.9, identity>(1-diff_threshold) * 100) 
...
p <- ggtree(dat$tree, color=colors[3], lwd=1) + 
  #geom_tippoint(size=4, color=colors[3]) +
  geom_tiplab(color=colors[3],hjust=1, fontface='bold', offset=0.05, align = TRUE) +
...
p1 <- p + 
  ...
  scale_fill_manual(values=colors[c(1,5,2,6,4)],na.value='white') +
  theme(legend.position = 'right', strip.background =element_rect(fill=NA, color=NA), 
        strip.text = element_text(size=10, face='bold'))
        
tree1 <- facet_widths(p1, c(2,0.5,0.5,1))
tree1
```
...
Plot the tree
```{r fig.height=2, fig.width=11, message=FALSE}
p <- ggtree(dat$tree, color=colors[1], lwd=1) + 
  #geom_tippoint(size=4, color=colors[3]) +
  geom_tiplab(color=colors[1],hjust=1, fontface='bold', offset=0.4, align = TRUE) +
  geom_treescale(x=0, y=3.4, color = colors[1], linesize=2) 
...
tree2 <- facet_widths(p1, c(2,0.5,0.5,0.5,0.5,0.5,1))
tree2
```
...
p <- ggtree(dat$tree, color=colors[4], lwd=1) + 
  #geom_tippoint(size=4, color=colors[3]) +
  geom_tiplab(color=colors[4],hjust=1, fontface='bold', offset=0.02, align = TRUE) +
...
p1 <- 
  p + geom_facet(panel='Nasser et al\nDerivative', data=d2, geom=geom_point, 
...
tree3 <- facet_widths(p1, c(2,0.5,0.5,0.5,1))
tree3
```
...
binom.test(filter(test.dat, MDR) %>% pull(`99.99%.y`) %>% sum(na.rm = T), 
           sum(test.dat$`99.99%.y`, na.rm = T), 
           filter(test.dat, MDR) %>% nrow / nrow(test.dat) ## MDR + overlap with patient
           )$p.value
...
p <- ggtree(dat$tree, color=colors[2], lwd=1) + 
  geom_tiplab(color=colors[2],hjust=1, fontface='bold', offset=0.02, align = TRUE) +
...
tree4 <- facet_widths(p1, c(2,0.5,0.5,1))
tree4
```
...
ggsave('../plots/fig5_phylogenetic_trees.svg', height=10, width=20)
```
