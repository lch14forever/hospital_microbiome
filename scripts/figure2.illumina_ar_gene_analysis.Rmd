...
mutate(anti, Cover=1*(Cover>0)) %>% 
  pivot_wider(id_cols = Anti, names_from = Lib, values_from = Cover, values_fill = list(Cover = 0)) %>% 
  write.table("../output_tables/illumina_AR_gene_assignment_matrix.dat", quote=F, row.names = F, col.names = T, sep="\t")
