...
get.clusters <- plot.tree <- function(x){
  ...
  merged <- merge(data.frame(clusters), meta.merged, by.x=0, by.y=1) %>%  
    mutate(strain=paste0('s', clusters), input_distance_matrix=x) 
  merged
}
...
plot.tree <- function(x, color, offset=0.01, title=NULL, scale_offset_x=0.005, scale_offset_y=0){
  ...
  ##cluster.strains <- hclust(as.dist(dist.dat[names(strains), names(strains)]), method='average')
  ## read precomputed parsnp tree
  tree.strains <- read.tree(paste0('../tables/trees/', str_match(x, '/([A-Z][a-z]+_[a-z]+)')[,2], ".parsnp.tree"))
  tree.strains$tip.label %<>% 
  ...
  p <- ggtree(as.phylo(cluster.strains), layout="fan", open.angle=45, lwd=1.5) %<+%
    ...
    geom_treescale(x=scale_offset_x, y=scale_offset_y, width=0.001, linesize=1, offset=0.1) 
...
```{r}
d1 <- get.clusters('../tables/Elizabethkingia_anophelis_mummer_heatmap.dat')
d2 <- get.clusters('../tables/Staphylococcus_epidermidis_mummer_heatmap.dat')
d3 <- get.clusters('../tables/Staphylococcus_aureus_mummer_heatmap.dat')
d4 <- get.clusters('../tables/Acinetobacter_baumannii_mummer_heatmap.dat')
d5 <- get.clusters('../tables/Pseudomonas_aeruginosa_mummer_heatmap.dat')
d6 <- get.clusters('../tables/Enterococcus_faecium_mummer_heatmap.dat')
d7 <- get.clusters('../tables/Enterococcus_faecalis_mummer_heatmap.dat')
d8 <- get.clusters('../tables/Klebsiella_pneumoniae_mummer_heatmap.dat')
write.table(rbind(d1, d2, d3, d4, d5, d6, d7, d8), 
...
g4 <- plot.tree('../tables/Acinetobacter_baumannii_mummer_heatmap.dat', colors[4], 0.007, 'Acinetobacter baumannii', scale_offset_x = 0.006)
g5 <- plot.tree('../tables/Pseudomonas_aeruginosa_mummer_heatmap.dat', colors[5], 0.004, 'Pseudomonas aeruginosa', scale_offset_x = 0.004)
g6 <- plot.tree('../tables/Enterococcus_faecium_mummer_heatmap.dat', colors[6], 0.0055, 'Enterococcus faecium', scale_offset_x = 0.018)
g7 <- plot.tree('../tables/Enterococcus_faecalis_mummer_heatmap.dat', colors[7], 0.007, 'Enterococcus faecalis', scale_offset_x = 0.018)
g8 <- plot.tree('../tables/Klebsiella_pneumoniae_mummer_heatmap.dat', colors[8], 0.006, 'Klebsiella pneumoniae', scale_offset_x = 0.018)
...
```{r}
test.dat <- rbind(g2$dat, g3$dat, g4$dat, g5$dat, g6$dat, g7$dat, g8$dat) %>% 
  group_by(Species, clusters, Sample_type, Room_type, bed_number,
           timept, Sample_ID.y, Cubicle_room) %>% 
...
ggsave('../plots/sup8.tree_sup.pdf', s1, width=54, height=9, limitsize = F)
pdf('../plots/sup8.hive_sup.pdf', width = 60, height = 10)
aux()
dev.off()
```
