...
tree3 <- gheatmap(p, anno, color='black', colnames_offset_y = 0.3, width=0.3, offset=0.001, font.size=3.5,
  ...
  coord_flip() +
  ylim(-0.5,27) +
  scale_fill_manual(values=colors[4], guide=F)
...
tree4 <- gheatmap(p, anno, color='black', colnames_offset_y = 0.3, width=0.5, offset=0.0004, font.size=3.5,
  ...
  scale_x_reverse() + 
  coord_flip() +
  scale_fill_manual(values=colors[2], guide=F)
...
p <- ggtree(tree, aes(color=group) , layout = 'circular', lwd=1.5) + 
  ##geom_text2(aes(subset=!isTip, label=node), hjust=-0.8) + 
  scale_color_manual(values=c("black", colors), guide=F) +
  geom_hilight(node=540, alpha=0.2, fill = colors[4]) +
  geom_hilight(node=535, alpha=0.2, fill = colors[1]) +
  geom_hilight(node=533, alpha=0.2, fill = colors[2]) +
