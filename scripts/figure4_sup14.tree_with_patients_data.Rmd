...
get_tree_data <- function(species, filter.patient.only=TRUE, outlier='', get.dist=FALSE){
  ...
  list(anno=anno, strains=strains, tree=midpoint(tree.strains))
}
...
p <- ggtree(tree, color=colors[3], lwd=1) %<+%
  ...
  geom_tiplab(color=colors[3],size=4, hjust=1, fontface='bold', offset=0.02) +
...
p <- ggtree(as.phylo(clustering), color=colors[1], lwd=1) %<+%
  strains +
  geom_tippoint(aes(size=n, color=lab)) +
  geom_tiplab(color=colors[1],size=4, hjust=1, fontface='bold', offset=5e-3) +
...
p <- ggtree(tree, color=colors[4], lwd=1) %<+%
  strains +
  geom_tippoint(aes(size=n, color=lab)) +
  geom_tiplab(color=colors[4],size=4, hjust=1, fontface='bold', offset=1.3e-3) +
  scale_size_continuous(trans='log2', limits=c(1,64)) + 
  scale_shape_manual(values=c(21,19)) + 
  scale_color_manual(values=c('grey','black')) + 
...
p <- ggtree(tree, color=colors[2], lwd=1) %<+%
  strains +
  geom_tippoint(aes(size=n, color=lab)) +
  geom_tiplab(color=colors[2],size=4, hjust=1, fontface='bold', offset=1.6e-3) +
  scale_size_continuous(trans='log2') + 
  scale_shape_manual(values=c(21,19)) + 
  scale_color_manual(values=c('grey','black')) + 
