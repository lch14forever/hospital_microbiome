...
p <- ggtree(as.phylo(clustering), color=colors[1], lwd=1) %<+%
  strains +
  geom_tippoint(aes(size=n, color=lab)) +
  geom_tiplab(aes(label=paste0('s', strain)), color=colors[1],size=4, hjust=1, fontface='bold', offset=3.4e-3) +
...
p <- ggtree(as.phylo(clustering), color=colors[4], lwd=1) %<+%
  ...
  scale_shape_manual(values=c(21,19)) + 
  scale_color_manual(values=c('grey','black')) + 
  geom_treescale(x=0, y=15, width=0.001, color = colors[4], linesize=2) 
## Add heatmap
dd <- rownames_to_column(anno) %>% melt(id.vars='rowname')
p1 <- facet_plot(p , "Antibiotics resistance profile", 
                 data=dd,
...
p <- ggtree(as.phylo(clustering), color=colors[2], lwd=1) %<+%
  strains +
  geom_tippoint(aes(size=n, color=lab)) +
  geom_tiplab(aes(label=paste0('s', strain)), color=colors[2],size=4, hjust=1, fontface='bold', offset=1.3e-3) +
  scale_size_continuous(trans='log2') + 
  scale_shape_manual(values=c(21,19)) + 
  scale_color_manual(values=c('grey','black')) + 
...
meta <- read.table('../output_tables/merged_assembly_metadata.tsv', head=TRUE) %>% 
  select(genomeID, Species, lib) %>% unite(id, Species, lib, remove = FALSE)
## remove duplicates
idx <- rownames(full_mat) %in% meta$id
full_mat <- full_mat[idx, idx]
rownames(full_mat) <- colnames(full_mat) <- meta$genomeID[match(rownames(full_mat), meta$id)]
