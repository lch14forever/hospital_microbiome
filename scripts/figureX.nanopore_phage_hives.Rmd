...
Merge data
```{r}
mat <- read.table('../tables/phage_mummer_heatmap.dat', head=TRUE, stringsAsFactors = FALSE, comment.char = '!', sep='\t')
...
```{r}
novel.phage <- read.table('../tables/novel_phages.dat', stringsAsFactors = F)[,1]

dat.sel <- mutate(dat.sel, isNovel=Row.names %in% novel.phage) 

write.table(dat.sel, '../output_tables/phage_selected_cluster_summary.tsv', row.names = F, col.names = T, sep='\t', quote=F)

novel_clusters <- select(dat.sel, clusters, tmp=isNovel) %>% 
  group_by(clusters) %>% 
