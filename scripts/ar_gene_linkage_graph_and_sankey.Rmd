...
edges.collapsed <- group_by(edges.full, X1, X2) %>% 
  tally() 
#filter(edges.full, dist<Inf) %>%
#    group_by(X1,X2) %>% tally() %>%
#    filter(n>2) -> edges.collapsed
```
...
## node size
#### carbapenemase
args <- c('MECA_Bla', 'CARB_Bla', 'OXA-48_Bla', 'OXA-228_Bla', 'IMP-1_Bla', 'CARB-5_Bla',
          'OXA-23_Bla','OXA-58_Bla','OXA-51_Bla')
...
g_cols <- c('burlywood', colors[2:5], 'purple',  'blue', 'darkgreen', 'pink', 'red', 'gold', 'black')
p <- ggraph(g, layout = 'fr')+#, circular=TRUE) +
  ...
  theme_void()
p
ggsave('../plots/ar_gene_linkage_graph.pdf', p, width=20, height=20)
```
...
select(dat, Species=V1, gene=V6) %>% mutate(gene=str_replace(gene, '_.*','')) %>%
...
p <- ggplot(sankey.dat,aes(y=log10(n), axis1=Species, axis2=Antibiotics_cluster)) +
  geom_alluvium(aes(fill=Antibiotics_cluster), width=1/12)  +
  geom_stratum(width=1/12) +
  geom_text(stat = "stratum", label.strata = TRUE, size=3) +
  scale_fill_manual(values=g_cols) +
  scale_x_discrete(limits = c("Species", "Antibiotics Cluster"), expand = c(.05, .05)) +
  theme_void()
p
ggsave('../plots/ar_gene_linkage_graph.pdf', p, width=10, height=15)
```
