...
get.clusters <- function(x, algo){
  dist.file <- paste0("../tables/", x, "_", algo, "_mummer_heatmap.dat")
  ...
  clusters <- cutree(cluster.full, h=0.00001)
  ## filter patient data
...
```{r}
d1 <- get.clusters('Elizabethkingia_anophelis', 'opera_ms')
d2 <- get.clusters('Staphylococcus_epidermidis', 'opera_ms')
d3 <- get.clusters('Staphylococcus_aureus', 'opera_ms')
d4 <- get.clusters('Acinetobacter_baumannii', 'opera_ms')
d5 <- get.clusters('Pseudomonas_aeruginosa', 'canu')
d6 <- get.clusters('Enterococcus_faecium', 'canu')
d7 <- get.clusters('Enterococcus_faecalis', 'canu') #%>% filter(Nanopore_ID != 'N182_BC10') 
## N182_BC10 is removed due to low coverage comparing to the reference when constructing the tree
d8 <- get.clusters('Klebsiella_pneumoniae', 'canu')
