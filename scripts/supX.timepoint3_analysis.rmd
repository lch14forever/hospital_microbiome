...
plot.dat <- foreach(r=1:nrow(pairing.dat), .combine = rbind) %do% {
    dat <- pairing.dat[r, ]
    dat$`to same type` <-  min(dist.mat[dat[, 3], filter(tp2, Type==dat$Type) %>% pull(id)])
    dat$`to opposite type` <- min(dist.mat[dat[, 3], filter(tp2, Type!=dat$Type) %>% pull(id)])
    dat
}
...
melt(plot.dat) %>% 
  ...
  ylim(c(0, 1.5) )+
  theme(axis.text.x = element_text(angle=40, hjust=1)) +
  stat_compare_means(comparisons = list(c("to same type", "to opposite type")),
                     paired=TRUE,
                     label="p.signif", size=13,
                     method.args = list(alternative = "less")) 
```