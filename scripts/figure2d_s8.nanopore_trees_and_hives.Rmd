...
levels(meta.merged$Sample_type) <- stringr::str_replace_all(levels(meta.merged$Sample_type), c('handle-interior'='Handle'))
...
plot.tree <- function(x, color, offset=0.01, title=NULL){
  ...
  p <- gheatmap(p, offset=offset, antibiotics, color='black', colnames_offset_y = 0.3,##colnames_offset_x=10,
    ...
    theme(legend.position ='none', plot.title = element_text(size = 20, face = "bold.italic")) + 
    ggtitle(title)
...
g7 <- plot.tree('../tables/Enterococcus_faecalis_mummer_heatmap.dat', colors[7], 0.007, 'Enterococcus faecalis')
