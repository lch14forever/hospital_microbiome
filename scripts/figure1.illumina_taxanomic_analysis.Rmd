...
fig1bi <- pheatmap(log(data2all+1,2), color=(colorRampPalette(brewer.pal(9,'Blues'))(50)),
 border_color="black",cluster_rows = F, cluster_cols = F, cellwidth =10, cellheight=10,
 gp=gpar(fontface = "bold.italic"), silent=TRUE)
cowplot::plot_grid(fig1bi$gtable)

ggsave('../plots/fig1bi_dry_wet_genus_comparison.pdf', 
       width = 4, height = 5)
```
...
ggsave('../plots/fig1bii_dry_wet_species_comparison.pdf', height = 5, width = 10)
```
...
ggsave('../plots/fig1d_distance_spatial_temporal.pdf', height = 8, width = 4)
## ad hoc analysis
#median(same_bed) 
...
### (Fig1f) Species differentially abundant in dry environments
...
ggplot(data = all.dat.dry.fil, aes(x=variable,y=value,fill=Room_type)) +
  ...
  theme(axis.text.x = element_text(angle=270, hjust=0, vjust=0.5, size=12, face = 'bold.italic'), 
        legend.position = c(0.5,0.8), legend.title = element_blank()) +
  scale_fill_npg() + 
  labs(y='Relative abundance (%)', x=NULL) +
  coord_cartesian(ylim =c(0,17))
ggsave('../plots/fig1f_species_in_dry_environments.pdf', height = 8, width = 5)
```
