...
p1 <- ggplot(data = all.dat.dry.fil, aes(x=variable,y=value,fill=Room_type)) +
  geom_boxplot(outlier.shape = NA, lwd=1) +
  theme(axis.text.x = element_text(angle=270, hjust=0, vjust=0.5, size=12, face = 'bold.italic'), 
        legend.position = c(0.5,0.92), legend.title = element_blank()) +
  scale_fill_npg() + 
  labs(y='Relative abundance (%)', x=NULL) +
  coord_cartesian(ylim =c(0,17))

p.zoom <- ggplot(data = all.dat.dry.fil, aes(x=variable,y=value,fill=Room_type)) +
  geom_boxplot(outlier.shape = NA, lwd=1) +
...
p1 +  
  annotation_custom(grob = ggplotGrob(p.zoom), xmin = 0.4, xmax = 3.8, ymin = 8, ymax = 14)
