...
get_tree_data <- function(species, filter.patient.only=TRUE, outlier=''){
  ...
  meta <- read.table('../metadata/merged_assembly_metadata.tsv', head=TRUE) %>% filter(Species==species)
  ...
  list(anno=anno, strains=strains, clustering=clustering, bars=bars)
}
```
...
p <- ggtree(as.phylo(clustering)) %<+%
  strains +
  geom_tippoint(aes(size=n), shape=19) +
  geom_tiplab(aes(label=paste0('s', strain)),size=4, color='black', fontface='bold', offset=5e-4) +
...
tree
ggsave('../plots/s_aureus_tree_w_patient_data.pdf', tree, width=10, height=10)
```
...
tree <- cowplot::plot_grid(l, tree + theme(legend.position="none"),
          rel_widths=c(0.1,1) , nrow=1)
tree
ggsave('../plots/e_anophelis_tree_w_patient_data.pdf', tree, width=10, height=10)
```
### 
...
p <- ggtree(as.phylo(clustering)) %<+%
  ...
  geom_tiplab(aes(label=paste0('s', strain)),size=4, color='black', fontface='bold', offset=5e-4) +
  scale_size_continuous(trans='log2') + 
  geom_treescale(x=0, y=40, width=0.001) +
  theme(legend.position="left")
...
tree <- cowplot::plot_grid(l, tree + theme(legend.position="none"),
          rel_widths=c(0.1,1) , nrow=1)
tree
ggsave('../plots/s_epidermidis_tree_w_patient_data.pdf', tree, width=10, height=15)
```
