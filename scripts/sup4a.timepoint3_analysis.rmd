...
p1.1 <- ggplot(hi.score, aes(x=Tp1, y=Tp2, color=Sample_type)) + 
  geom_point(size=3) + 
  scale_y_log10() + 
  scale_x_log10() +
  labs(x='Timepoint 1', y='Timepoint 2') + 
...
p2.1 <- ggplot(hi.score, aes(x=Tp2, y=Tp3, color=Sample_type)) + 
  geom_point(size=3) + 
  scale_y_log10() +
  scale_x_log10() + 
  labs(x='Timepoint 2', y='Timepoint 3') + 
...
cowplot::plot_grid(p1.1 + theme(legend.position='none'), 
                   p2.1 + theme(legend.position='none'), 
                   nrow=1)

p.all <- cowplot::plot_grid(p1.1 + theme(legend.position='none'), 
                   p2.1 + theme(legend.position='none'), 
                   p1 + theme(legend.position='none'),
                   p2 + theme(legend.position='none'),
                   align = 'hv',
                   nrow=2)
ggsave('../plots/sup4ab.human_influence_site_specificity.png', p.all, height = 8, width = 9)
...
plot.dat <- foreach(r=1:nrow(pairing.dat), .combine = rbind) %do% {
    dat <- pairing.dat[r, ]
    dat$`Same\nCommunity` <-  min(dist.mat[dat[, 3], filter(tp2, Type==dat$Type) %>% pull(id)])
    dat$`Different\nCommunity` <- min(dist.mat[dat[, 3], filter(tp2, Type!=dat$Type) %>% pull(id)])
    dat
}

mutate(plot.dat) %>% melt %>% 
  ...
  labs(x=NULL, y='Bray-Curtis Dissimilarity\n(timepoint 3 versus 1)') +
  theme(strip.background = element_blank() ) +
  stat_compare_means(comparisons = list(c("Same\nCommunity", "Different\nCommunity")),
                     paired=TRUE,
                     label="p.signif", size=13,
                     method.args = list(alternative = "less")) 

ggsave('../plots/sup3b.strain_persistance.png', height = 5, width = 9)
```