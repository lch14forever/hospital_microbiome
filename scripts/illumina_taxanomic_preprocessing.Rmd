...
mutate(tmp, species=str_replace_all(species, c('s__'='', '_'=' '))) %>% 
  ...
  gghighlight(ratio>3 & batch1>0.22 | (batch2>0.22 & ratio<1/3)) +
...
mat <- cor(t(batch1[tmp$batch1>0.22,]), method='spearman')

rownames(mat) %<>% str_replace_all(c('s__'='', '_'=' ')) %>% str_replace('[a-z]+ ', '. ')
colnames(mat) %<>% str_replace_all(c('s__'='', '_'=' ')) %>% str_replace('[a-z]+ ', '. ')
idx <-  which(rownames(batch1[tmp$batch1>0.22,]) %in% species.removed)
