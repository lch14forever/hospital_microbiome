...
<!-- ```{r} -->
<!-- df.nanopore <- read.table('../tables/nanopore.kraken.table.s',head=T,sep='\t',row.names=1, quote='&', comment.char='&') -->
...
<!-- o <- c('BHI', 'AMP', 'CHLOR', 'KAN', 'STREP', 'TET') -->
<!-- nanopore_final <- mutate(nanopore_final, Antibiotics=factor(Antibiotics,level=o, ordered = T)) -->

<!-- g.b <- ggplot(data=nanopore_final, aes(x=variable,y=freq,fill=Antibiotics)) + -->
<!--   geom_bar(width=0.8,color="black",position="dodge",stat="identity") + -->
<!--   theme(axis.text.x = element_text(angle=40, hjust=1, size=10, face='bold.italic') ) + -->
<!--   scale_fill_manual(values=pal_simpsons(c("springfield"))(16)[-c(2,8)]) -->
<!-- g.b -->
<!-- ``` -->
...
ggsave('../plots/sup6.pdf', g.a, width = 10, height = 5)
```
